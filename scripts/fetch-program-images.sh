...
fetch() { # <program-id> <url> [cropH cropW offsetY offsetX]
    ...
    # Optional crop, in source pixels, for portrait sources that would lose their
    # subject to the centered landscape crop on cards and the detail hero.
    if [ $# -eq 6 ]; then
        sips -c "$3" "$4" --cropOffset "$5" "$6" "$tmp" >/dev/null 2>&1
    fi
...
fetch 3d-pen-studio          "$base/5/54/Person_using_3D_printing_pen_at_exhibition_%281%29.jpg"
...
# Portrait source: crop to 4:3 around the bridge and its textbook load.
fetch structures-skyscrapers "$base/3/3c/Success%21.jpg" 2264 3019 1000 0
fetch lab-coat-science       "$base/7/74/Outreach_with_girls.jpg"
